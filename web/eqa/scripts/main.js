...
function getRootPath(){
	return System.IO.Path.Combine(System.Environment.get_CurrentDirectory(),"web","eqa");
}
...
function preparePager(id, column, table, callback, prefix){
	...
	var pagerList = [];
	var con = new SqlLiteConnection(System.IO.Path.Combine(getRootPath(),"databases","beqas_dict.db"),"","");
	if(con != null){
		con.open();
	...
	if(System.IO.File.Exists(sub_file)){
		...
		System.IO.File.ReadAllText(sub_file, function(content){
			setTimeout(function(){ 
				...
				$(".BEQAS-Title").html("Basic External Quality Assessment System v"+getVersion());
				//closeLoadingPage();				
			},1000);
		});					
	}
